import { Component } from '@angular/core';

@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css']
})
export class AppComponent {
  title = 'Base 64 to Json Formatter';

  inputText:string = '';
  outputText:any = undefined;
  test:boolean = false;

  ngDoCheck(){
    this.encodeDecode();
  }

  encodeDecode(){
    let regex = /^([0-9a-zA-Z+/]{4})*(([0-9a-zA-Z+/]{2}==)|([0-9a-zA-Z+/]{3}=))?$/;
    let test = regex.test(this.inputText);
    // this.outputText = test ? JSON.stringify(JSON.parse(atob(this.inputText)),null,4) : 'Invalid Encoded Input';
    this.outputText = test ? JSON.parse(atob(this.inputText)) : 'Invalid Encoded Input';
  }
}
